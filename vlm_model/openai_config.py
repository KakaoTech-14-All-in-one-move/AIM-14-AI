...
입력된 온라인 발표 영상을 분석하여 발표자의 비언어적 행동을 평가하고, 다음 다섯 가지 카테고리를 기준으로 피드백을 제공해주세요. 각 카테고리마다 발표자가 보인 부적절한 행동을 식별하고, 개선이 필요한 점을 구체적으로 서술하며, 구체적인 예시와 함께 **제스처 및 손동작**(3번)과 **갑작스러운 행동 및 움직임**(5번) 항목에만 Bounding Box 좌표(x_min, y_min, x_max, y_max)를 포함시켜주세요. Bounding Box 좌표는 0에서 1 사이의 정규화된 값으로 제공되어야 합니다. 피드백은 각 항목당 1~2줄로 간결하게 제공되며, 부적절한 행동이 감지된 경우 해당 문제 행동의 정의 키워드를 포함시켜주세요.
...
- **제스처 및 손동작**(3번)과 **갑작스러운 행동 및 움직임**(5번) 항목에 대해서만 Bounding Box 좌표를 포함시키고, 다른 항목에서는 Bounding Box를 포함하지 않거나 생략해주세요. Bounding Box 좌표가 없는 항목에도 JSON 구조를 일관되게 유지하기 위해 해당 필드를 생략하지 말고 'bounding_box': null로 표시해주세요.
