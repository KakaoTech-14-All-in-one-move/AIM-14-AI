...
입력된 온라인 발표 영상에서  분석을 통해 감지된 비언어적 행동의 점수와 함께 발표자의 비언어적 행동을 평가하고, 다음 네 가지 카테고리를 기준으로 피드백을 제공해주세요. 얼굴 표정 (facial_expression)은 점수가 제공되지 않으므로, 영상 분석 결과에 따라 System Instruction의 지침을 기반으로 판단하여 평가합니다.
각 카테고리마다 발표자가 보인 부적절한 행동을 식별하고, 개선이 필요한 점을 구체적으로 서술하며, 구체적인 예시와 함께 피드백은 각 항목당 1~2줄로 간결하게 제공되며, 구체적인 예시와 함께 권장 사항을 제공합니다. 피드백은 각 항목당 1~2줄로 간결하게 작성하며, 부적절한 행동이 감지된 경우 해당 문제 행동의 정의 키워드를 포함시켜주세요.
문제가 없는 경우에는 해당 항목을 피드백에서 제외하거나 "문제가 없음"으로 간단히 표기합니다.
...
- **0.3 ~ 0.6**: 경고 (Mild Issue) - 약간의 개선이 필요합니다.
- **0.6 ~ 1.0**: 심각함 (Severe Issue) - 즉각적인 개선이 필요합니다.
...
- **0.3 ~ 0.6**: 가끔 카메라를 응시하지 않거나, 화면 밖으로 시선 이동이 관찰됨.
- **0.6 ~ 1.0**: 시선이 지속적으로 불안정하거나, 화면 밖을 자주 응시함.

#### 2. 제스처 (gestures)
- **0.0 ~ 0.3**: 손동작이 발표와 적절히 연계되어 있음. 문제 없음.
- **0.3 ~ 0.6**: 손동작이 약간 과도하거나 불필요한 동작이 간헐적으로 관찰됨.
- **0.6 ~ 1.0**: 손동작이 과도하여 시각적으로 산만하거나, 발표 내용을 방해함.

#### 3. 자세 및 신체 언어 (posture_body)
- **0.0 ~ 0.3**: 자세가 안정적이고, 발표 흐름에 방해되지 않음. 문제 없음.
- **0.3 ~ 0.6**: 자세가 약간 구부정하거나, 자세 변화가 관찰됨.
- **0.6 ~ 1.0**: 자세가 구부정하거나 자주 변화하며, 발표의 안정성을 방해함.

#### 4. 갑작스러운 행동 및 움직임 (movement)
- **0.0 ~ 0.3**: 움직임이 적절하며, 발표와 조화를 이룸. 문제 없음.
- **0.3 ~ 0.6**: 약간의 갑작스러운 움직임이 가끔 관찰됨.
- **0.6 ~ 1.0**: 움직임이 과도하거나, 발표 흐름을 심각하게 방해함.
